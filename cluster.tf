...
# 3. Auto Scaling Group (Mixed: 1 On-Demand Base)
resource "aws_autoscaling_group" "ecs" {
  ...
  mixed_instances_policy {
    instances_distribution {
      on_demand_base_capacity                  = 1 # Keeps 1 stable On-Demand instance
      on_demand_percentage_above_base_capacity = 0 # All scaling is Spot
  ...
  lifecycle {
    # FIX: Prevents Terraform from fighting with AWS Managed Scaling
